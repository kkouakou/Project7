...
def main():
    ...
    st.set_page_config(page_title='Tableau de bord de notation des demandes de prêt',
                       page_icon='random',
                       layout='centered',
                       initial_sidebar_state='auto')

    # Display the title
    st.title('Tableau de bord de notation des demandes de prêt')
    st.header("Kouakou Kevin - Projet de science des données 7")
    ...
    select_sk_id = st.sidebar.selectbox('Sélectionnez SK_ID dans la liste:', SK_IDS, key=18)
    ...
    def get_list_display_features_importance(shap_val_trans, def_n, key):
        count_checkbox_importance = 0        
        all_feat = X_tr_all.columns.to_list()
        ...
        if st.checkbox('Choose main features according to SHAP local interpretation for the applicant customer', key=count_checkbox_importance):
            disp_cols = list(shap_val_trans.abs()
                                .sort_values(ascending=False)
                                .iloc[:n].index)
        else:
    ...
    if st.sidebar.checkbox("Notation et décision du modèle", key=38):

        st.header("Notation et décision du modèle")
...
for a random sample of customers from the training set.")
    ...
    if st.sidebar.checkbox('Boîtes à moustaches des principales caractéristiques', key=23):

        st.header('Boîtes à moustaches des principales caractéristiques')

        plt.clf()

        with st.spinner('Création de boxplots en cours...'):
...
their loan, and red boxplots are for the customers that didn't repay it.Over the boxplots are\
 ...
 Values for the applicant customer are superimposed in yellow.")
    ...
    if st.sidebar.checkbox('Comparaison du nuage de points', key=27):
    
        st.header('Comparaison du nuage de points')
        
        plt.clf()
        
        disp_box_cols = get_list_display_features(shap_val_trans, 2, key=41)
        if len(disp_box_cols)>2:
            proj = st.radio("Choisissez la méthode de projection", ['PCA', 't-SNE'])
        else:
            proj = 'PCA'
...
Values for the applicant customer are superimposed in yellow.")
 ...
 is shown.")
    ...
    if st.sidebar.checkbox("Importance des fonctionnalités", key=29):

        st.header("Comparaison des importances locales et globales des fonctionnalités")
        ...
        disp_box_cols_importance = get_list_display_features_importance(shap_val_trans, 10, key=42)
        ...
        expander = st.expander("Concerning the comparison of local and global feature importances...")
...
20 nearest neighbors for each feature, as well a the corresponding value of this feature (colormap).")
    ...
    if st.sidebar.checkbox('Description des fonctionnalités', key=22):

        st.header("Description des fonctionnalités")
